            ...
            <div className="mt-14 flex flex-col space-y-5">
                {
                    user?.role == 'client' ?
                        dashboardNavLinks.map((navItem, index) => (
                            <a href={navItem.path} key={navItem.index} className={`cursor-pointer flex items-center space-x-3 p-3 rounded-lg ${activeNavIndex === index ? "bg-[#fd961a] text-white rounded-l-xl" : " "}`}>
